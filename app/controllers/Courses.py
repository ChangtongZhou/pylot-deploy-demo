...
class Courses(Controller):
    ...
    # We should set up a GET route for this method
    # def showOne(self, id):
    ...
    def add(self):
        ...
        if len(request.form['title']) == 0:
            flash("Course name cannot be empty, please enter a course name!")
            error = True
        elif len(request.form['title']) < 15:
    ...
    def delete(self, course_id):
        courses = self.models['Course'].get_course_by_id(course_id)
        return self.load_view('delete.html', one_course = courses)

    def delete_process(self, course_id):
